...
namespace ScraperLib
{
    ...
    {
        private readonly Lazy<PricePage> page = new Lazy<PricePage> (() => new PricePage (new PriceBrowser()));

        private DayPrices ParseSingleDay(PageData data, int dayOffset)
        {
        ...
        async public Task<IEnumerable<DayPrices>> FetchWeekPrices(DateTime date)
        {
            var data = await page.Value.GetPageDataAsync(date);
            return Enumerable
